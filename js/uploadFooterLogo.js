...
const folderPath = "totfd/Footerlogo-images";
...
async function updateFirestoreWithSelectedURL(imageName) {
    try {
        const url = await getDownloadURL(
            ref(storage, "totfd/Footerlogo-images/" + imageName)
        );

        await updateDoc(Homepage, {
            footerlogoimageurl: url,
        });

        document.getElementById("message").textContent = "FooterLogo Image Updated.";
        window.scrollTo(0, 0);
        setTimeout(function () {
